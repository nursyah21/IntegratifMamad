...
const fetchDataTransaksi = async (auth,id) => {
...
const schema = Yup.object({
  ...
  function ListData({token = AUTH, role='', data=[], setData}){
    ...
    const MyDialog = ({props=dialogProps}) => {
      ...
      useEffect(()=>{
        if(props.id.id){
          fetchDataTransaksi(token.accessToken,  props.id.id).then(e=>{
            setUserData(e)
    ...
    const handleSearch = (event) => {
      if (event.key === "Enter") {
        ...
        if(val === '') {
          ...
          fetchDataTransaksi(token.accessToken, val)
          .then(data=>{
            const temp = []
            temp.push(data)
            setData(temp)
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 mb-16">
              <thead className="bg-gray-50 dark:bg-gray-800">
                <tr className='text-left'>
                  ...
                  <th scope="col" className="px-4 py-3.5 text-sm font-normal text-left rtl:text-right text-gray-500 dark:text-gray-400">Kendaraan</th>
                  <th scope="col" className="px-4 py-3.5 text-sm font-normal text-left rtl:text-right text-gray-500 dark:text-gray-400">Penyewa</th>
                  <th scope="col" className="px-4 py-3.5 text-sm font-normal text-left rtl:text-right text-gray-500 dark:text-gray-400">Pengembalian</th>
                </tr>
              </thead>
